...
- 클라이언트의 사용자 경험 최적화
  ...
  클라이언트는 브라우저의 뷰포인트 외부에 있는 이미지를 썸네일로 로드한 뒤 해당 이미지가 뷰포인트 내부로 진입 시에 원본 이미지를 로드합니다. 더불어 이미지를 30개 단위로 로드하고 스크롤에 따라서 연속적으로 이미지를 추가 로드합니다. 클라이언트의 보기 옵션(정렬기준, 태그)이 변경될 경우 기존에 다운로드 중인 이미지의 REST 요청을 즉각 취소합니다. 테마(라이트/다크)와 이미지 크기 조절 옵션을 지원합니다. 또한 반응형 웹으로 모바일과 태블릿, 데스크탑 모든 기기에 대응합니다.
